    ...
    >
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="fixed  inset-x-0 flex flex-col gap-1"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
...
const FloatingDockDesktop = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden min-h-16 gap-4 items-end rounded-2xl bg-gray-50 dark:bg-neutral-900 px-4 pb-3 md:flex md:flex-col  pt-3 mt-3 fixed",
        className
      )}
      z
    >
